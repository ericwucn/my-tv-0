...
class ProgramAdapter(
    ...
    class ViewHolder(private val context: Context, private val binding: ProgramItemBinding) :
        ...
        fun bindTitle(epg: EPG) {
            ...
            val beginDay = Utils.getDateFormat("yyyy-MM-dd", epg.beginTime)
            val todayDay = Utils.getDateFormat("yyyy-MM-dd", now.toInt())
            val yesterdayDay = Utils.getDateFormat("yyyy-MM-dd", (now - 86400).toInt())

            // ??????????????
            beginTimeStr = when {
                beginDay == todayDay -> Utils.getDateFormat("HH:mm", epg.beginTime)
                beginDay == yesterdayDay -> "?? ${Utils.getDateFormat("HH:mm", epg.beginTime)}"
                else -> Utils.getDateFormat(dateFormat + " HH:mm", epg.beginTime)
            }
            endTimeStr = Utils.getDateFormat("HH:mm", epg.endTime)

            binding.title.text = "$beginTimeStr-$endTimeStr"
            binding.description.text = epg.title

            // ===== ???? =====
            // ??????(???)??????
            val isReplayable = epg.endTime < now
            val isCurrent = epg.beginTime <= now && epg.endTime > now

            if (isReplayable) {
                // ??? - ????:?????
                binding.description.alpha = 0.8f
            } else if (isCurrent) {
                // ????
                binding.description.alpha = 1.0f
            } else {
                // ???
                binding.description.alpha = 0.6f
            }
        }
